...
#heading
Label(root, text = "Youtube Video Downloader", font = 'arial 40 bold').pack()

#url entry

Label(root, text = "Paste the link here", font = 'arial 25 bold' , fg='#C21010').pack()
Label(root, text = "(for downloading multiple videos use comma(',') between links)", font = 'arial 10 bold' , fg='#C21010').pack()
link = StringVar()
link_enter = Entry(root, textvariable = link, width = 70).pack()

#url error message
urlErr = Label(root, font = 'arial 12', fg = 'purple')
urlErr.pack()

# # storage button
Label(root, text="Select the destination folder", font = 'arial 12 bold', fg = '#C21010').pack(pady=5)
...
#download button
Button(root, text="DOWNLOAD", fg = 'white', bg = '#c4302b', width = 17, height=2, command = lambda: _thread.start_new_thread(download, ())).pack(pady = 10)
